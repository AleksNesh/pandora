...
//            return true;
    ...
    private function _setItemsDiscount()
    {
        ...
        foreach ($items as $item) {
            ...
            if ($itemPrice - $itemDiscountAmount < $availableDiscount) {
                $discount = $itemPrice - $itemDiscountAmount;
                $item->setDiscountAmount($discount + $itemDiscountAmount);
                $item->setOriginalDiscountAmount($discount + $itemDiscountAmount);
                $item->setRowTotalWithDiscount($item->getRowTotal()-($discount+ $itemDiscountAmount));
            ...
            if ($baseItemPrice - $baseItemDiscountAmount < $baseAvailableDiscount) {
                $baseDiscount = $baseItemPrice - $baseItemDiscountAmount;
                $item->setBaseDiscountAmount($baseDiscount + $baseItemDiscountAmount);
                $item->setBaseOriginalDiscountAmount($baseDiscount + $baseItemDiscountAmount);
