...
/**
 * class Alpine_PrintPdf_Model_PdfCustomiser_Invoice
 *
 * @category   Alpine
 * @package    Alpine_PrintPdf
 */

class Alpine_PrintPdf_Model_PdfCustomiser_Invoice extends Fooman_PdfCustomiser_Model_Invoice
{
        ...
        foreach ($orderIds as $key => $orderId) {
            ...
            if ($invoices->getSize() > 0) {
                foreach ($invoices as $invoice) {
                    ...
                    $showGC = Alpine_PrintPdf_Model_PdfCustomiser_Order::showGiftWrapCardInfo($order->getStoreId());
                    $showGWM = Alpine_PrintPdf_Model_PdfCustomiser_Order::showGiftWrapMessage($order->getStoreId());
                    $giftWrapTemplate = 'gift-wrap-items.phtml';
                    $giftBoxCollection = Alpine_PrintPdf_Model_PdfCustomiser_Order::getGiftBoxCollection($invoice);
