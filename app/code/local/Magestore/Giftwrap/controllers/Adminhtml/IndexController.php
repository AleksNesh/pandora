...
//    }
        ...
        if ($wraptype && count($data['wrap']) > 0) {
            ...
            if ($wraptype == 1) {
                ...
                if ($number_items > $maxItems) {
                    Mage::getSingleton('adminhtml/session_quote')->addError('Number items in giftbox is too large!');
                    $this->_redirect('adminhtml/sales_order_create/index/');
                    return;
                }
                /* if($data['giftbox_id'])
                ...
                try {
                    $model->setId(null);
                    $model->save();
                    $this->_getSession()->addSuccess($this->__('Added gift wrap successfully.'));
                } catch (Exception $e) {
    ...
    public function deletegiftboxAction() {
        ...
        try {
            $giftbox->delete();
            $this->_getSession()->addSuccess($this->__("Deleted gift wrap successfully."));
        } catch (Exception $e) {
            $this->_getSession()->addError($e->getMessage());
        }
        $quote = Mage::getSingleton('adminhtml/session_quote')->getQuote();
