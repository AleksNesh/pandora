var OrdersProEdit = Class.create();
OrdersProEdit.prototype = {
    ...
    loadEditForm : function(blockId, element) {
        ...
        new Ajax.Request(url, {
            onComplete: function(transport) {
                ...
                oldFieldset.insert({after: '<div class="fieldset" id="orderspro-fieldset">' + transport.responseText + '</div>'});
    ...
    cancel : function() {
        $$('#orderspro-fieldset').each(function(el){
            el.remove();
        });
    ...
    getFormParams : function() {
        ...
        if (this.currentBlock == 'payment_method') {
            ...
            var fieldset = $('orderspro-fieldset');
            ...
            $('orderspro_edit_form').select('input', 'select', 'textarea').each(function (el) {
                if (el.checked || (el.type != 'checkbox' && el.type != 'radio')) {
    ...
    searchProducts : function() {
        new Ajax.Request(this.productGridUrl, {
            onComplete : function(transport) {
                var gridEl = $('orderspro_product_grid');
                if (gridEl) {
    ...
    showPopup : function(content) {
        var overlay = $('orderspro-overlay');
        var popup = new Element('div');
        popup.id = 'orderspro-popup';
        ...
        var posLeft = (width - $('orderspro-popup').getWidth()) / 2;
        popup.style.left = posLeft + 'px';

        var height = window.innerHeight || document.documentElement.clientHeight;
        var posTop = (height - $('orderspro-popup').getHeight()) / 2;
        popup.style.top = posTop + 'px';
    },

    hidePopup : function() {
        $$('#orderspro-overlay').each(function(el){
            el.remove();
        });

        $$('#orderspro-popup').each(function(el){
            el.remove();
        });
    },
...
var OrdersProEditItems = Class.create();
OrdersProEditItems.prototype = {
